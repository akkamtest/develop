...
/*	
	#ifdef CONFIG_SYS_SUPPORT_64BIT_DATA
		start = simple_strtoull(argv[3], NULL, 16);
	#else
	...
	*/
	...
	switch (test) {
		case 0:
			...
			printf("addr_tst0: stop option = %d, start = %llx, end = %llx\n", stop, start, end);
			for (i = 1; i <= MEMTEST_ITERATION;i++)
			{
				printf("addr_tst0 iter = %d\n", i);
				ret |= addr_tst0(buf, start, end, stop);
			}
			//mem_test_quick(buf, start, end, 0xCAFEDECA, 1);
			printf("addr_tst1: stop option = %d, start = %llx, end = %llx\n", stop, start, end);
			for (i = 1; i <= MEMTEST_ITERATION;i++)
			{
				printf("addr_tst1 iter = %d\n", i);
				ret |= addr_tst1(buf, start, end, stop);
			}
			
			printf("movinv: stop option = %d, start = %llx, end = %llx, number of iteration = %d\n", stop, start, end, MEMTEST_ITERATION);
			ret |= movinv (MEMTEST_ITERATION, buf, start, end, stop);
			
			printf("movinv_8bit: stop option = %d, start = %llx, end = %llx, number of iteration = %d\n", stop, start, end, MEMTEST_ITERATION);
			ret |= movinv_8bit (MEMTEST_ITERATION, buf, start, end, stop);
			
			printf("movinvr: stop option = %d, start = %llx, end = %llx, number of iteration = %d\n", stop, start, end, MEMTEST_ITERATION);
			ret |= movinvr (MEMTEST_ITERATION, MEMTEST_RAND_SEED_1, buf, start, end, stop);
			
			printf("movinv64: stop option = %d, start = %llx, end = %llx\n", stop, start, end);
			for (i = 1; i <= MEMTEST_ITERATION;i++)
			{
				printf("movinv64 iter = %d\n", i);
				ret |= movinv64(buf, start, end, stop);
			}
			
			printf("rand_seq: stop option = %d, start = %llx, end = %llx\n", stop, start, end);
			for (i = 1; i <= MEMTEST_ITERATION;i++)
			{
				printf("rand_seq iter = %d\n", i);
				ret |= rand_seq(i, buf, start, end, stop);
			}
			
			printf("modtst: stop option = %d, start = %llx, end = %llx\n", stop, start, end);
			for (i = 0; i <= MOD_SZ-1 ;i++)
			{
				printf("modtst iter = %d\n", i);
				ret |= modtst(i, 20, 0xA5A5A5A5A5A5A5A5, 0x5A5A5A5A5A5A5A5A, buf, start, end, stop);
			}
			
			printf("bit_fade_fill iter = %d\n", 1);
			bit_fade_fill(MEMTEST_PATTERN_64_A, buf, start, end);
			wait(2);
			bit_fade_chk(MEMTEST_PATTERN_64_A, buf, start, end, stop);
			break;
		case 1:
			printf("addr_tst0: stop option = %d, start = %llx, end = %llx\n", stop, start, end);
			for (i = 1; i <= MEMTEST_ITERATION;i++)
		...
		case 4:
			...
			break;
		case 5:
			printf("move_block: stop option = %d, start = %llx, end = %llx\n", stop, start, end);
			// TODO
			break;
		case 6:
			printf("movinvr: stop option = %d, start = %llx, end = %llx, number of iteration = %d\n", stop, start, end, MEMTEST_ITERATION);
		...
		case 9:
			...
			{
				printf("modtst iter = %d\n", i);
				ret |= modtst(0, MEMTEST_ITERATION, 0xA5A5A5A5A5A5A5A5, 0x5A5A5A5A5A5A5A5A, buf, start, end, stop);
			}			
			break;
		case 10:
			printf("bit_fade: stop option = %d, start = %llx, end = %llx\n", stop, start, end);
			bit_fade_fill(0xCAFEDECACAFEDECA, buf, start, end);
			wait(5);
			printf("wait\n");
			ret |= bit_fade_chk(0xCAFEDECACAFEDECA, buf, start, end, stop);
			break;
		default:
			return CMD_RET_USAGE;
			break;
